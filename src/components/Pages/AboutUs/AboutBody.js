...
const Expart=(props)=>{
   const {img,name}=props
    return(
      <div className="md:w-80 bg-white rounded-lg sahdow-lg overflow-hidden flex flex-col justify-center items-center  box-sha">
      <div>
        
          <img className="object-center object-cover h-auto w-full" src={img} alt="" />
      </div>
      <div className="text-center py-8 sm:py-6">
          <p className="text-xl text-gray-700 font-bold mb-2">{name}</p>
          <p className="text-base text-gray-400 font-normal">Travel Agent</p>
      </div>
  </div>
    )
}
